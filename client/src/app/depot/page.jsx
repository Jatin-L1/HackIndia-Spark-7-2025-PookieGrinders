...
useEffect(() => {
  ...
  if (activeDelivery && activeDelivery.status === 'in-progress' || activeDelivery?.status === 'authenticated') {
...
const generateOTP = async () => {
  try {
    ...
    setMetaMaskModalMessage("Setting OTP and transitioning delivery to IN_TRANSIT...");
    ...
    try {
      ...
      for (let i = 1; i <= Number(deliveryCount); i++) {
        try {
          ...
          if (delivery.deliveryPersonId.toString() === activeDelivery.deliveryPersonId && 
      ...
      setOtpSuccess(`OTP ${randomOtp} generated successfully! The delivery person can now verify this OTP.`);
      
      // Update delivery status to reflect IN_TRANSIT state
      const updatedDelivery = {...activeDelivery, status: 'in-progress', otpGenerated: true};
    ...
    try {
      ...
      for (let i = 1; i <= Number(deliveryCount); i++) {
        try {
          ...
          if (delivery.deliveryPersonId.toString() === activeDelivery.deliveryPersonId) {
            console.log("Found delivery:", i, "Status:", delivery.status.toString());
            
            // Update UI based on contract status if needed
            const statusNum = Number(delivery.status);
            
            if (statusNum === 1 && activeDelivery.status !== "authenticated") {
              setActiveDelivery({
                ...activeDelivery,
                status: "authenticated"
              });
              setOtpSuccess('OTP was successfully verified by the delivery person!');
            } else if (statusNum === 2 && activeDelivery.status !== "location-verified") {
              setActiveDelivery({
                ...activeDelivery,
...
const completeDelivery = async (deliveryId = null) => {
  try {
    ...
    try {
      ...
      setSuccess('Delivery completed successfully! Payment of 0.01 ETH has been sent to the delivery person.');
...
const verifyLocation = async () => {
  try {
    ...
    setMetaMaskModalMessage("Verifying location and sending verification payment...");
    setShowMetaMaskModal(true);
    ...
    try {
      ...
      setSuccess('Location verified successfully! You can now proceed with ration distribution.');
    } catch (error) {
      setShowMetaMaskModal(false);
      throw error;
    }
  ...
  //   }
  // };
  // Updated allocation function with delivery person and ETH amount
...
//       let paymentAmount = ethers.parseEther("0");
//       if (ethAmount && parseFloat(ethAmount) > 0) {
//         paymentAmount = ethers.parseEther(ethAmount);
//       }
      
//       // Call contract to track ration distribution with payment if needed
//       const trackTx = await contract.allocateRation(
//         userId,
//         depotId,
//         deliveryPersonId || '0',
...
//         txHash: trackTx.hash,
//         timestamp: Date.now(),
//         details: `Ration allocated to User ID: ${userId}${deliveryPersonId && deliveryPersonId !== '0' ? ` via Delivery Person ID: ${deliveryPersonId}` : ''}${parseFloat(ethAmount) > 0 ? ` with payment of ${ethAmount} ETH` : ''}`
//       });
...
//       // Show success message
//       setSuccess(`Ration allocation to ${user ? user.name : 'User ' + userId} completed successfully!${parseFloat(ethAmount) > 0 ? ` Payment of ${ethAmount} ETH included.` : ''}`);
...
// Update your existing trackRationDistribution function
const trackRationDistribution = async (userId, deliveryPersonId = '0', ethAmount = '0', items) => {
  try {
    setLoading(true);
...
//              metaMaskModalType === 'reset' ? 'Reset Delivery' : 
//              'Delivery Processing'}
//           </span>
//         </div>
                ...
                <Card className="border-green-100 shadow-sm">
                  ...
                  <CardContent>
                    <form onSubmit={(e) => {
                      ...
                      const formData = new FormData(e.target);
                      const beneficiaryId = formData.get('beneficiaryId');
                      const deliveryPersonId = formData.get('deliveryPersonId');
                      ...
                      trackRationDistribution(beneficiaryId, deliveryPersonId, ethAmount, items);
                      e.target.reset();
                    }}>
                      <div className="grid grid-cols-1 gap-6">
                        <div className="grid gap-3">
                        ...
                        <div className="grid gap-3">
                          <Label htmlFor="deliveryPersonId">Select Delivery Person</Label>
                          <Select name="deliveryPersonId">
                            <SelectTrigger id="deliveryPersonId" className="w-full">
                              <SelectValue placeholder="Choose a delivery person (optional)" />
                            </SelectTrigger>
                            <SelectContent>
                              <SelectItem value="0">None (Direct Distribution)</SelectItem>
                              {assignedDeliveryPersons.map(person => (
                        ...
                        <div className="grid gap-3">
                          ...
                          <Input
                            ...
                            defaultValue="0"
                          />
                          <p className="text-xs text-gray-500">Optional payment to include with allocation</p>
                        </div>
                        ...
                        <TableBody>
                          {rationDistributions.map((ration) => (
                            <TableRow key={ration.id} className="hover:bg-green-50/50">
                              ...
                              <TableCell>{ration.userName}</TableCell>
                              <TableCell>{ration.category}</TableCell>
                              <TableCell>{ration.deliveryPersonName || 'Direct'}</TableCell>
                              <TableCell>{formatDate(ration.date)}</TableCell>
                              <TableCell>
                                {ration.ethAmount ? (
                                  <span className="text-green-700 font-medium">{ration.ethAmount} ETH</span>
                                ) : (
          ...
          <div className="border-t border-b py-4 my-4">
            <div className="flex justify-between mb-2">
              ...
              <span className="font-medium">
                {metaMaskModalType === 'otp' ? 'Generate OTP' : 
                 metaMaskModalType === 'reset' ? 'Reset Delivery' : 
                 metaMaskModalType === 'allocation' ? 'Ration Allocation' :
                 'Delivery Processing'}
              </span>
            </div>
